...
print("dates",df.select(pl.col("date").min(),pl.col("date").max()))
print("timestamp",df.select(pl.col("timestamp").min(),pl.col("timestamp").max()))
print("types",df.group_by("option_type").len().sort("len"))
print("expiry range",df.select(pl.col("expiry").min(),pl.col("expiry").max()))
