...
def feature_query(root:Path)->str:
    ...
    SELECT trade_date,datetime,expiry_type,MIN(expiry) AS first_expiry,spot,call_iv,put_iv,
    ...
    GROUP BY ALL ORDER BY trade_date,datetime,expiry_type
    '''
...
def select_signals(features,variant):
    ...
    x=features[m].copy()
    if x.empty: return x
    x.expiry=x.groupby(['trade_date','expiry_type'])['first_expiry'].transform('min')
    return x.sort_values(['trade_date','datetime']).drop_duplicates(['trade_date','expiry_type'])[['trade_date','datetime','expiry_type','expiry','spot','vrp','abs_ret15']]

def load_entry_quotes(root,signals):
    if signals.empty: return pd.DataFrame()
    con=duckdb.connect(); con.register('setups',signals[['trade_date','expiry_type','expiry']].drop_duplicates())
    g=parquet_glob(root)
    q=f'''
    SELECT CAST(o.datetime AS TIMESTAMP) AS datetime, CAST(o.date AS DATE) AS trade_date, o.expiry_type, CAST(o.expiry AS DATE) AS expiry, o.option_type, o.strike_type, CAST(o.strike_price AS DOUBLE) AS strike, CAST(o.open AS DOUBLE) AS open, CAST(o.high AS DOUBLE) AS high, CAST(o.low AS DOUBLE) AS low, CAST(o.close AS DOUBLE) AS close
    FROM read_parquet('{g}', union_by_name=true) o JOIN setups s ON CAST(o.date AS DATE)=s.trade_date AND o.expiry_type=s.expiry_type AND CAST(o.expiry AS DATE)=s.expiry
    WHERE o.close>0 AND o.option_type IN ('CALL','PUT') AND o.strike_type IN ('ATM','ATM+2','ATM-2')
    '''
...
def simulate(signal,setup,bars,structure,stop_ratio,slippage):
    ...
    ex=bars[bars.datetime==exit_ts].iloc[-1]; lot=index_option_lot_size('NIFTY',signal.expiry)
    cm=OptionCostModel()
...
def run(data_root,out,slippage):
    features=load_signal_rows(data_root); variants=variant_grid(); trades=[]
    for v in variants:
        sig=select_signals(features,v)
        if sig.empty: continue
        eq=load_entry_quotes(data_root,sig)
        if eq.empty: continue
        for _,signal in sig.iterrows():
            setup=pick_setup(eq,signal)
