...
def signals(feat,v):
    ...
    return feat.loc[m].sort_values('datetime').drop_duplicates(['trade_date','expiry_type','expiry'])
...
def run(root,out,slippage,expiry):
    ...
    outcome_rows=[]
    setup_index=setup_df.set_index(['trade_date','entry_time','expiry','expiry_type','side','width'])
    for key,u in setup_index.iterrows():
        d,et,expiry,ex,side,width=key
        q=win[(win.trade_date==d)&(win.entry_time==et)&(win.expiry==u.expiry)&(win.expiry_type==ex)&(win.side==side)&(win.short_strike==u.short_strike)&(win.wing_strike==u.wing_strike)]
        if q.empty: continue
        ...
        for hold in HOLDS:
            ...
            for stop in STOPS:
                ...
                outcome_rows.append({
                    'trade_date':d,'datetime':et-pd.Timedelta(minutes=1),'expiry':expiry,'expiry_type':ex,
    ...
    if not outcomes.empty:
        for v in vs:
            ...
            keycols=['trade_date','datetime','expiry','expiry_type']
            oo=outcomes[
