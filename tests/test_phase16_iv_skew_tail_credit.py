...
def test_feature_query_selects_nearest_future_expiry(tmp_path):
    from research.phase16_iv_skew_tail_credit import feature_query
    for name in [
        "ATM+2_CE.parquet","ATM+2_PE.parquet","ATM-2_CE.parquet","ATM-2_PE.parquet",
        "ATM+3_CE.parquet","ATM+3_PE.parquet","ATM-3_CE.parquet","ATM-3_PE.parquet",
        ...
        p.write_bytes(b"")
    q=feature_query(tmp_path,"WEEK")
    assert "MIN(expiry) OVER(PARTITION BY datetime, trade_date, expiry_type)" in q
    assert "WHERE expiry=nearest_expiry" in q
    assert "CAST(expiry AS DATE) expiry" in q


def test_hold_window_is_exact():
    import pandas as pd
    from research.phase16_iv_skew_tail_credit import clip_hold_window
    start=pd.Timestamp("2026-01-01 04:00:00")
    m=pd.DataFrame({"datetime":[start+pd.Timedelta(minutes=i) for i in range(61)]})
    assert len(clip_hold_window(m,start,30)) == 31
    assert len(clip_hold_window(m,start,60)) == 61
