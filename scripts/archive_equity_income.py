...
def fetch_youtubegpt(video_id: str):
    """Fetch the source caption track through the documented hosted JSON endpoint.

    This is a source-preserving fallback: the service returns caption segments
    with millisecond timings and a generated/human track flag; no transcript
    text is generated by this repository.
    """
    ...
    try:
        ...
        for seg in data.get("segments") or []:
            ...
            try:
                ...
                continue
            try:
                duration = float(dur_ms) / 1000.0 if dur_ms is not None else 0.0
            except (TypeError, ValueError):
                duration = 0.0
        ...
        return snippets, lang, str(track.get("name") or lang), generated
...
def fetch_transcript(video_id: str, video_url: str, retries: int = 1):
    ...
    1) documented no-key caption proxy (fastest path around GitHub/YouTube IP blocks),
    2) curated Invidious caption APIs,
    3) yt-dlp subtitle extraction with several YouTube player clients,
    4) youtube-transcript-api,
    5) direct InnerTube caption tracks,
    6) direct timedtext.
    """
    errors = []

    # Try the documented no-key caption service first. It explicitly falls back
    # from human captions to auto-generated captions and returns timestamped text.
    ...
    if youtubegpt[0] is not None:
        snippets, lang, label, generated = youtubegpt
        return snippets, lang, label, generated, "youtubegpt-json"
    ...
    errors.append("invidious:all_instances_failed")

    ytdlp_result = fetch_ytdlp_subtitles(video_url)
    if ytdlp_result[0] is not None:
        return ytdlp_result
    errors.extend(f"yt-dlp:{err}" for err in ytdlp_result[1])
    ...
    errors.append("youtube-innertube:failed")

    timed = fetch_timedtext(video_id)
    if timed is not None:
        snippets, lang, generated = timed
        return snippets, lang, lang, generated, "youtube-timedtext"
    errors.append("youtube-timedtext:failed")

    detail = " | ".join(errors[-16:])
