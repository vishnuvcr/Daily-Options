...
SIGMAS = (1.0, 2.0)
ADJUSTMENTS = ('NONE', 'REDUCE_AT_50PCT', 'REDUCE_AT_75PCT')
...
def build_setup(con, index_path, expiry_path, expiry, entry_date, entry_clock, sigma, strike_cache, vix):
    ...
    if ce_series.empty or pe_series.empty:
        return None
    if price_at(ce_series, fill_ts) is None or price_at(pe_series, fill_ts) is None:
        return None
    return {
        'expiry': expiry, 'entry_date': entry_date, 'entry_clock': entry_clock,
        'signal_ts': signal_ts, 'fill_ts': fill_ts, 'exit_ref': exit_ref,
        'spot': spot, 'vix': vix_value, 'sigma': sigma, 'expected_move': move,
        'strikes': {'CE': ce, 'PE': pe}, 'series': {'CE': ce_series, 'PE': pe_series},
        'spot_series': spot_series, 'lot': lot_size(expiry),
    }

def simulate_trade(setup, adjustment, slippage, brokerage):
    lot = setup['lot']
    cash = 0.0
    costs = 0.0
    active = {'CE': True, 'PE': True}
    entry_ts = setup['fill_ts']
    exit_ref = setup['exit_ref']
    spot0 = setup['spot']
    ce_strike = setup['strikes']['CE']
    pe_strike = setup['strikes']['PE']
    frac = adjustment_fraction(adjustment)
    ce_trigger = spot0 + frac * (ce_strike - spot0) if frac is not None else None
    pe_trigger = spot0 - frac * (spot0 - pe_strike) if frac is not None else None
    for side in ('CE','PE'):
        px = price_at(setup['series'][side], entry_ts)
        if px is None:
            return None
        cash += execution_cash(px, 'SELL', 1, lot, slippage)
        costs += order_cost(px, 'SELL', 1, lot, entry_ts.date(), brokerage)
    adjustments_done = 0
    for _, row in setup['spot_series'].iterrows():
        ts = pd.Timestamp(row['ts'])
        if ts < entry_ts or ts > exit_ref or adjustment == 'NONE' or adjustments_done >= 2:
            continue
        spot = float(row['close_px'])
        challenged = []
        if active['CE'] and spot >= ce_trigger:
            challenged.append('CE')
        if active['PE'] and spot <= pe_trigger:
            challenged.append('PE')
        if not challenged:
            continue
        exec_ts = ts + pd.Timedelta(minutes=1)
        for side in challenged:
            if not active[side] or adjustments_done >= 2:
                continue
            px = price_at(setup['series'][side], exec_ts)
            if px is None:
                return None
            cash += execution_cash(px, 'BUY', 1, lot, slippage)
            costs += order_cost(px, 'BUY', 1, lot, exec_ts.date(), brokerage)
            active[side] = False
            adjustments_done += 1
    ...
    for side in ('CE','PE'):
        if not active[side]:
            continue
        px = price_at(setup['series'][side], exit_ts)
        if px is None:
            return None
        cash += execution_cash(px, 'BUY', 1, lot, slippage)
        costs += order_cost(px, 'BUY', 1, lot, exit_ts.date(), brokerage)
    return {
        'expiry': str(setup['expiry']), 'entry_date': str(setup['entry_date']),
        'entry_clock': setup['entry_clock'], 'sigma': setup['sigma'],
        'adjustment': adjustment, 'trigger_fraction': frac,
        'spot': setup['spot'], 'vix': setup['vix'], 'expected_move': setup['expected_move'],
        'ce_strike': ce_strike, 'pe_strike': pe_strike, 'lot': lot,
        'adjustments': adjustments_done, 'costs': float(costs),
...
def run(data_root, out_root, slippage, brokerage):
    ...
    for expiry, path in files:
        for offset in ENTRY_OFFSETS:
            ...
            for clock in ENTRY_CLOCKS:
                for sigma in SIGMAS:
                    setup = build_setup(con, index_path, path, expiry, entry_date, clock, sigma, strike_cache, vix)
                    if setup is None:
                        continue
                    setup['entry_offset'] = offset
                    for adjustment in ADJUSTMENTS:
                        result = simulate_trade(setup, adjustment, slippage, brokerage)
                        if result is None:
                            continue
                        result['entry_offset'] = offset
                        result['cell_id'] = f"AD30_{abs(offset):02d}_{clock.replace(':','')}_S{int(sigma)}_{adjustment}"
                        rows.append(result)
    con.close()
    ...
    if trades.empty:
        ...
        return summary
    key_cols = ['expiry','entry_offset','entry_clock','sigma','adjustment']
    dup = int(trades.duplicated(key_cols).sum())
    if dup:
        raise RuntimeError(f'duplicate setup keys: {dup}')
    trades.to_csv(out/'trades.csv', index=False)
    group_cols = ['cell_id','entry_offset','entry_clock','sigma','adjustment']
    ...
    for keys,g in trades.groupby(group_cols, dropna=False, sort=False):
        ...
        extra.append({
            'cell_id':keys[0], 'entry_offset':int(keys[1]), 'entry_clock':keys[2], 'sigma':float(keys[3]), 'adjustment':keys[4],
    ...
    summary={'status':'COMPLETE','slippage':slippage,'brokerage':brokerage,'cell_count':int(board['cell_id'].nunique()),'reference_position_size':'1 NIFTY lot short CE + 1 NIFTY lot short PE','weekly_target':WEEKLY_TARGET,'preliminary_pass_cells':int(board['preliminary_pass'].sum()),'duplicate_setup_keys':dup,'best':best,'coverage':coverage}
