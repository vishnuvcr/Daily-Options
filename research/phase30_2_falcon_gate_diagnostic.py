...
CHECKS=[("2025-09-09","2025-09-16","2025-09-03"),("2025-09-16","2025-09-23","2025-09-10"),("2025-09-23","2025-09-30","2025-09-17")]
...
def load(con,expiry,d):
    ...
    WHERE upper(underlying)='NIFTY' AND granularity='1min'
      ...
      AND CAST(date AS DATE)=DATE '{d}'
      AND CAST(timestamp AS TIMESTAMP) BETWEEN TIMESTAMP '{d} 09:00:00' AND TIMESTAMP '{d} 15:15:00'
...
def inspect(con,ne,fe,d):
    ...
    for t in TIMES:
        ...
        for p in TARGETS:
            ...
            item={"near":None,"far_same":None,"far_diag":None}
            if ce is not None and pe is not None:
                item["near"]={"ce_strike":float(ce.strike),"pe_strike":float(pe.strike),"ce_close":float(ce.close_px),"pe_close":float(pe.close_px)}
                for name,outward in (("far_same",False),("far_diag",True)):
                    fce,fpe=pick(fs,"CE",p,ce.strike,outward),pick(fs,"PE",p,pe.strike,outward)
                    if fce is None or fpe is None:
                        item[name]={"selectable":False}; continue
                    rows=[nf[(nf.option_type=="CE")&(nf.strike==ce.strike)],
                          nf[(nf.option_type=="PE")&(nf.strike==pe.strike)],
                          ff[(ff.option_type=="CE")&(ff.strike==fce.strike)],
                          ff[(ff.option_type=="PE")&(ff.strike==fpe.strike)]]
                    if any(z.empty for z in rows):
                        item[name]={"selectable":True,"fill_complete":False,"far_strikes":[float(fce.strike),float(fpe.strike)]}; continue
                    sc,sp,fc,fp=[float(z.iloc[0].open_px) for z in rows]
                    credit=5*(sc+sp)-3*(fc+fp)
                    item[name]={"selectable":True,"fill_complete":True,"far_strikes":[float(fce.strike),float(fpe.strike)],"prices":[sc,sp,fc,fp],"credit":credit,"positive_credit":bool(credit>0)}
            rec["targets"][str(p)]=item
        out["times"][t]=rec
    return out
